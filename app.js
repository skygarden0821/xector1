...
(function setupCrop() {
  ...
  function go() {
    ...
    const m=(x,y)=>{
      if(!crop.drag)return;
      crop.x+=x-crop.lx; crop.y+=y-crop.ly; crop.lx=x; crop.ly=y;
      if(!pend){pend=true;requestAnimationFrame(()=>{drawCrop();pend=false;});}
    };
    ...
    c.addEventListener('touchmove', ev=>{
      if(ev.touches.length===2){
        ...
        if(ld)crop.scale=Math.max(.5,Math.min(4,crop.scale*(d/ld)));
        ld=d; drawCrop();
    ...
    c.addEventListener('wheel',ev=>{
      crop.scale=Math.max(.5,Math.min(4,crop.scale*(ev.deltaY<0?1.08:.93)));
...
function setFilter(k) {
  activeFilter = k; renderFilters(); renderTips();
}
...
function openTip(id) {
  ...
  document.getElementById('tip-content').innerHTML = `
    ...
    <ul class="sheet-list">
      ${t.points.map(p=>`<li style="--c:${t.color}">${p}</li>`).join('')}
    </ul>
...
function init() {
  ...
  document.getElementById('member-days').textContent = `在籍${days}日目`;
